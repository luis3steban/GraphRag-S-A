...
# t2c_llm = Ollama(model="llama3.1", temperature=0.1)
embedding_model = OpenAIEmbeddings(model="text-embedding-ada-002")
llm = ChatOpenAI(temperature=0, model_name='gpt-4o', streaming=False)
t2c_llm = ChatOpenAI(temperature=0, model_name='gpt-3.5-turbo', streaming=True)
...
RESPONDE EN ESPAÑOL Y DIRECTAMENTE SOLO LA RESPUESTA EN BASE A LA PREGUNTA.
en base al context que generas de lo que te pido, respondete al input una respuesta detallada y util.
genera una buena sugerencia en base al input.
no realizes SET en las consultas solo realiza consultas en base al imput.
